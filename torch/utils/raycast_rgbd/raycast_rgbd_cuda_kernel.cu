...
bool getVoxel(const SdfData sdf, int batch, const float3& pos, float& dist, float3& color, float3& normal, float14& semantic) {
	...
	if (pi.x >= 0 && pi.y >= 0 && pi.z >= 0 && pi.x < sdf.dimx && pi.y < sdf.dimy && pi.z < sdf.dimz) {
        ...
        semantic.c0 = sdf.vals_semantic[idx*14+0];
        semantic.c1 = sdf.vals_semantic[idx*14+1];
        semantic.c2 = sdf.vals_semantic[idx*14+2];
        semantic.c3 = sdf.vals_semantic[idx*14+3];
        semantic.c4 = sdf.vals_semantic[idx*14+4];
        semantic.c5 = sdf.vals_semantic[idx*14+5];
        semantic.c6 = sdf.vals_semantic[idx*14+6];
        semantic.c7 = sdf.vals_semantic[idx*14+7];
        semantic.c8 = sdf.vals_semantic[idx*14+8];
        semantic.c9 = sdf.vals_semantic[idx*14+9];
        semantic.c10 = sdf.vals_semantic[idx*14+10];
        semantic.c11 = sdf.vals_semantic[idx*14+11];
        semantic.c12 = sdf.vals_semantic[idx*14+12];
        semantic.c13 = sdf.vals_semantic[idx*14+13];
		return true;
	}
	return false;
}
...
bool trilinearInterpolationSimpleFastFast(const SdfData sdf, int batch, const float3& pos, float& dist, float3& color, float3& normal, float14& semantic) {
	...
	if (!getVoxel(sdf, batch, posDual+make_float3(0.0f, 0.0f, 0.0f), vsdf, vcolor, vnormal, vsemantic)) return false;
	dist += (1.0f-weight.x)*(1.0f-weight.y)*(1.0f-weight.z)*vsdf;

	if (!getVoxel(sdf, batch, posDual+make_float3(oSet, 0.0f, 0.0f), vsdf, vcolor, vnormal, vsemantic)) return false;
	dist += weight.x *(1.0f-weight.y)*(1.0f-weight.z)*vsdf;

	if (!getVoxel(sdf, batch, posDual+make_float3(0.0f, oSet, 0.0f), vsdf, vcolor, vnormal, vsemantic)) return false;
	dist += (1.0f-weight.x)*weight.y *(1.0f-weight.z)*vsdf;

	if (!getVoxel(sdf, batch, posDual+make_float3(0.0f, 0.0f, oSet), vsdf, vcolor, vnormal, vsemantic)) return false;
	dist += (1.0f-weight.x)*(1.0f-weight.y)*weight.z*vsdf;

	if (!getVoxel(sdf, batch, posDual+make_float3(oSet, oSet, 0.0f), vsdf, vcolor, vnormal, vsemantic)) return false;
	dist += weight.x*weight.y *(1.0f-weight.z)*vsdf;

	if (!getVoxel(sdf, batch, posDual+make_float3(0.0f, oSet, oSet), vsdf, vcolor, vnormal, vsemantic)) return false;
	dist += (1.0f-weight.x)*weight.y *weight.z*vsdf;

	if (!getVoxel(sdf, batch, posDual+make_float3(oSet, 0.0f, oSet), vsdf, vcolor, vnormal, vsemantic)) return false;
	dist += weight.x*(1.0f-weight.y)*weight.z*vsdf;

	if (!getVoxel(sdf, batch, posDual+make_float3(oSet, oSet, oSet), vsdf, vcolor, vnormal, vsemantic)) return false;
...
#pragma unroll 1
	for(uint i = 0; i<3; i++)
	{
		...
		float cDist;
		if(!trilinearInterpolationSimpleFastFast(sdf, batch, worldCamPos+c*worldDir, cDist, color, normal, semantic)) return false;
			...
			if(lastSample.weight > 0 && ((lastSample.sdf > 0.0f && dist < 0.0f) || (lastSample.sdf < 0.0f && dist > 0.0f))) // current sample is always valid here
			{
				...
				if(b && abs(lastSample.sdf - dist) < params.threshSampleDist)
				{
					if(abs(dist) < params.threshSampleDist)
					{
						...
						raycastData.image_semantic[(dTid.z*params.width*params.height + dTid.y*params.width+dTid.x)*14 + 0] = semantic2.c0;
						raycastData.image_semantic[(dTid.z*params.width*params.height + dTid.y*params.width+dTid.x)*14 + 1] = semantic2.c1;
						raycastData.image_semantic[(dTid.z*params.width*params.height + dTid.y*params.width+dTid.x)*14 + 2] = semantic2.c2;
						raycastData.image_semantic[(dTid.z*params.width*params.height + dTid.y*params.width+dTid.x)*14 + 3] = semantic2.c3;
						raycastData.image_semantic[(dTid.z*params.width*params.height + dTid.y*params.width+dTid.x)*14 + 4] = semantic2.c4;
						raycastData.image_semantic[(dTid.z*params.width*params.height + dTid.y*params.width+dTid.x)*14 + 5] = semantic2.c5;
						raycastData.image_semantic[(dTid.z*params.width*params.height + dTid.y*params.width+dTid.x)*14 + 6] = semantic2.c6;
						raycastData.image_semantic[(dTid.z*params.width*params.height + dTid.y*params.width+dTid.x)*14 + 7] = semantic2.c7;
						raycastData.image_semantic[(dTid.z*params.width*params.height + dTid.y*params.width+dTid.x)*14 + 8] = semantic2.c8;
						raycastData.image_semantic[(dTid.z*params.width*params.height + dTid.y*params.width+dTid.x)*14 + 9] = semantic2.c9;
						raycastData.image_semantic[(dTid.z*params.width*params.height + dTid.y*params.width+dTid.x)*14 + 10] = semantic2.c10;
						raycastData.image_semantic[(dTid.z*params.width*params.height + dTid.y*params.width+dTid.x)*14 + 11] = semantic2.c11;
						raycastData.image_semantic[(dTid.z*params.width*params.height + dTid.y*params.width+dTid.x)*14 + 12] = semantic2.c12;
						raycastData.image_semantic[(dTid.z*params.width*params.height + dTid.y*params.width+dTid.x)*14 + 13] = semantic2.c13;
...
__global__ void raycast_rgbd_cuda_kernel(
	...
	if (x < params.width && y < params.height /*&& batch < batch_size*/) {
        ...
        for (int i = 0; i < 14; i++) {
            raycastData.image_semantic[(batch*params.width*params.height + y*params.width + x)*14 + i] = MINF;
        }

		const float4x4 curViewMatrixInv = *(float4x4*)(viewMatrixInv + batch*16);
		float3 camDir = normalize(kinectProjToCamera(
                params.depthMin, params.depthMax, params.getMx(batch), params.getMy(batch),
...
__global__ void raycast_rgbd_cuda_backward_kernel(
	...
	const float* __restrict__ grad_semantic,
	...
	if (x >= 0 && y >= 0 && z >= 0 && x < dimx && y < dimy && z < dimz && tidx < maxPixelsPerVoxel /*&& batch < batch_size*/) {
		...
		if (idx >= 0) {
			...
			if (tidx < num) {
                ...
                for (int i = 0; i < 14; i++) {
                    const float vals = grad_semantic[(batch*(image_height*image_width) + py*image_width + px)*14 + i] / (float)min(num,maxPixelsPerVoxel);
                    atomicAdd(&d_semantic[idx*14 + i], vals);
                }
			} // has mapping
		} // valid loc
	} // in 3d dims
...
void raycast_rgbd_cuda_forward(
    ...
    sdf.vals_semantic = vals_semantic.data<float>();
	...
	raycastData.image_semantic = imageSemantic.data<float>();
...
void raycast_rgbd_cuda_backward(
	...
	cutilSafeCall(cudaMemset(d_semantic.data<float>(), 0, sizeof(float)*d_semantic.size(0)*d_semantic.size(1)));
#ifdef _DEBUG
		cutilSafeCall(cudaDeviceSynchronize());
		cutilCheckMsg(__FUNCTION__);
#endif
