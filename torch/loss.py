...
def compute_color_inpaint_loss(target_colors, output_coarse_color, output_color, known_mask, weight_color):
    ...
    locs_coarse = output_coarse_color[0][:, -1] * dims[0] * dims[1] * dims[2] + output_coarse_color[0][:, 0] * dims[1] * \
                  dims[2] + output_coarse_color[0][:, 1] * dims[2] + output_coarse_color[0][:, 2]
    locs = output_color[0][:, -1] * dims[0] * dims[1] * dims[2] + output_color[0][:, 0] * dims[1] * dims[2] + \
           output_color[0][:, 1] * dims[2] + output_color[0][:, 2]
...
def filter_proj_target(raycast_color, color_thresh, color_space):
    if color_space == 'rgb':
        ...
        invalid = (raycast_color[:, :, :, 0] < thresh) & (raycast_color[:, :, :, 1] < thresh) & (
                    raycast_color[:, :, :, 2] < thresh)
    elif color_space == 'lab':
        thresh = 0.01
        invalid = (raycast_color[:, :, :, 0] < thresh) & (raycast_color[:, :, :, 1] > 0.5 - thresh) & (
                    raycast_color[:, :, :, 1] < 0.5 + thresh) & (raycast_color[:, :, :, 2] > 0.5 - thresh) & (
                              raycast_color[:, :, :, 2] < 0.5 + thresh)
    else:
        raise
    return invalid
...
class GANLoss:
    ...
    @staticmethod
    def compute_discriminator_loss_vanilla(ref_disc, in_real, in_fake, valid, weight, val_mode=False,
                                           label_smoothing_factor=1, alpha=1):
        ...
        if valid is not None:
            d_real = d_real[valid]
            d_fake = d_fake[valid]
        real_loss = torch.nn.functional.binary_cross_entropy_with_logits(d_real, torch.ones(
            d_real.shape).cuda() * label_smoothing_factor, reduction='none')
        real_loss = torch.mean(real_loss, 1)
        fake_loss = torch.nn.functional.binary_cross_entropy_with_logits(d_fake, torch.zeros(d_fake.shape).cuda(),
                                                                         reduction='none')
    ...
    @staticmethod
    def compute_discriminator_loss_wasserstein(ref_disc, in_real, in_fake, valid, weight, val_mode=False,
                                               label_smoothing_factor=1, alpha=1):
        d_real = ref_disc(in_real, alpha)
        d_fake = ref_disc(in_fake, alpha)
        if weight is not None:
    ...
    @staticmethod
    def compute_discriminator_loss_hinge(ref_disc, in_real, in_fake, valid, weight, val_mode=False,
                                         label_smoothing_factor=1, alpha=1):
        d_real = ref_disc(in_real, alpha)
        d_fake = ref_disc(in_fake, alpha)
        if weight is not None:
    ...
    @staticmethod
    def compute_discriminator_loss_wasserstein_gp(ref_disc, in_real, in_fake, valid, weight, val_mode=False,
                                                  label_smoothing_factor=1, alpha=1):
        if not val_mode:
            real_loss, fake_loss, _ = GANLoss.compute_discriminator_loss_wasserstein(ref_disc, in_real, in_fake, valid,
                                                                                     weight, val_mode, alpha)
            return real_loss, fake_loss, GANLoss.compute_gradient_penalty(ref_disc, in_real, in_fake)
        else:
            return GANLoss.compute_discriminator_loss_wasserstein(ref_disc, in_real, in_fake, valid, weight, val_mode,
                                                                  alpha)
    ...
    def compute_gradient_penalty(ref_disc, in_real, in_fake):
        ...
        gradients = torch.autograd.grad(outputs=prob_interpolated, inputs=interpolated,
                                        grad_outputs=torch.ones(prob_interpolated.size()).cuda(in_real.device.index),
