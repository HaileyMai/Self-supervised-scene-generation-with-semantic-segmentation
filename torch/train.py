...
args.num_iters_before_semantic = 40
...
if args.weight_disc_loss > 0 or args.weight_style_loss > 0 or args.weight_content_loss > 0 or args.weight_depth_loss > 0:
    ...
    depth2normals = Depth2Normals(args.batch_size, args.style_width, args.style_height, depth_min=0.1 / args.voxelsize,
                                  depth_max=raycast_depth_max / args.voxelsize,
                                  max_num_fill_iters=args.max_depth_fill_iters)

    raycaster_occ = RaycastOcc(args.batch_size, args.input_dim, args.style_width, args.style_height,
                               depth_min=0.1 / args.voxelsize, depth_max=4.0 / args.voxelsize,
...
if args.weight_disc_loss > 0:
    nf_in = 3 * (args.weight_depth_loss > 0) + 3 * (args.weight_color_loss > 0) + (
            3 * (args.weight_depth_loss > 0) + 3 * (args.weight_color_loss > 0))
    disc = model_util.Discriminator2D(nf_in=nf_in, nf=args.nf_disc, patch_size=args.patch_size,
                                      image_dims=(args.style_height, args.style_width), patch=args.patch_disc,
                                      use_bias=True, disc_loss_type=args.disc_loss_type).cuda()
...
# data files
train_files, val_files, _OVERFIT = data_util.get_train_files(args.data_path, args.train_file_list, args.val_file_list,
                                                             args.max_num_trainval_chunks)
...
train_dataset = scene_dataloader.SceneDataset(train_files, args.input_dim, args.truncation, args.use_loss_masking,
                                              args.augment_rgb_scaling,
                                              (args.augment_scale_min, args.augment_scale_max), args.color_truncation,
                                              args.color_space, args.frame_info_path, args.frame_path,
                                              image_dims=(args.style_width, args.style_height),
                                              load_depth=args.weight_depth_loss > 0,
                                              load_semantic=args.weight_semantic_loss > 0,
                                              subsamp2d_factor=args.subsample_tgt2d_factor,
                                              randomize_frames=args.randomize_frames, num_overfit=num_overfit_train)
print('train_dataset', len(train_dataset))
train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True,
                                               num_workers=args.num_workers, collate_fn=scene_dataloader.collate_voxels)
if len(val_files) > 0:
    val_dataset = scene_dataloader.SceneDataset(val_files, args.input_dim, args.truncation, args.use_loss_masking,
                                                args.augment_rgb_scaling,
                                                (args.augment_scale_min, args.augment_scale_max), args.color_truncation,
                                                args.color_space, args.frame_info_path, args.frame_path,
                                                image_dims=(args.style_width, args.style_height),
                                                ...
                                                load_semantic=args.weight_semantic_loss > 0,
                                                subsamp2d_factor=args.subsample_tgt2d_factor,
                                                randomize_frames=args.randomize_frames, num_overfit=num_overfit_val)
...
def print_log(log, epoch, iter, train_losses, train_lossoccs, train_iouoccs, train_losssdfs, train_lossdepths,
    ...
    mean_train_loss = -1 if (len(train_losses) == 0 or np.all(train_losses < 0)) else np.mean(
        train_losses[train_losses >= 0])
    mean_train_occloss = -1 if (len(train_lossoccs) == 0 or np.all(train_lossoccs < 0)) else np.mean(
        train_lossoccs[train_lossoccs >= 0])
    mean_train_occiou = -1 if (len(train_iouoccs) == 0 or np.all(train_iouoccs < 0)) else np.mean(
        train_iouoccs[train_iouoccs >= 0])
    mean_train_sdfloss = -1 if (len(train_losssdfs) == 0 or np.all(train_losssdfs < 0)) else np.mean(
        train_losssdfs[train_losssdfs >= 0])
    mean_train_depthloss = -1 if (len(train_lossdepths) == 0 or np.all(train_lossdepths < 0)) else np.mean(
    ...
    mean_train_styleloss = -1 if (len(train_lossstyles) == 0 or np.all(train_lossstyles < 0)) else np.mean(
        train_lossstyles[train_lossstyles >= 0])
    mean_train_contentloss = -1 if (len(train_losscontents) == 0 or np.all(train_losscontents < 0)) else np.mean(
        train_losscontents[train_losscontents >= 0])
    mean_val_loss = None
    ...
    if val_losses:
        ...
        mean_val_occloss = -1 if (len(val_lossoccs) == 0 or np.all(val_lossoccs < 0)) else np.mean(
            val_lossoccs[val_lossoccs >= 0])
        mean_val_occiou = -1 if (len(val_iouoccs) == 0 or np.all(val_iouoccs < 0)) else np.mean(
            val_iouoccs[val_iouoccs >= 0])
        mean_val_sdfloss = -1 if (len(val_losssdfs) == 0 or np.all(val_losssdfs < 0)) else np.mean(
            val_losssdfs[val_losssdfs >= 0])
        mean_val_depthloss = -1 if (len(val_lossdepths) == 0 or np.all(val_lossdepths < 0)) else np.mean(
        ...
        mean_val_styleloss = -1 if (len(val_lossstyles) == 0 or np.all(val_lossstyles < 0)) else np.mean(
            val_lossstyles[val_lossstyles >= 0])
...
def train(epoch, iter, dataloader, log_file, output_save):
    ...
    for t, sample in enumerate(dataloader):
        ...
        weight = loss_util.compute_dense_geo_weights(target_for_sdf, input_occ, args.truncation, args.weight_surf_geo,
        ...
        if pred_color:
            ...
            output_color = None
        if pred_semantic:  # TODO valid?
            assert sample['semantics'] is not None
            target_for_semantics = sample['semantics']
            pred_semantic = output_semantic[locs[:, -1], :, locs[:, 0], locs[:, 1], locs[:, 2]]
            target_for_semantics = target_for_semantics[locs[:, -1], :, locs[:, 0], locs[:, 1],
                                   locs[:, 2]].squeeze().cuda()
            pred_semantic = pred_semantic[target_for_semantics.detach() < 41]
            target = target_for_semantics[target_for_semantics.detach() < 41]
            loss_semantic = torch.nn.functional.cross_entropy(pred_semantic, target, weight=args.weight_semantic_class)
        ...
        if iter > args.num_iters_geo_only and len(output_sdf[0]) > 0 and (
            ...
            if True:
                ...
                input_normals = loss_util.compute_normals(inputs[:, :1], input_locs,
                                                          transform=torch.inverse(view_matrix))
                raycast_color, _, raycast_normal = raycaster_rgbd(input_locs.cuda(), input_vals[:, :1].contiguous(),
            ...
            if args.project_targets:
                ...
                if args.weight_by_percent_pixels:
                    weight_sample_tgt2d = 1.0 - torch.sum(invalid[:, :, :, 0].view(invalid.shape[0], -1),
            ...
            # raycast prediction            
            raycast_color, raycast_depth, raycast_normal = raycaster_rgbd(output_sdf[0].cuda(), output_sdf[1], color,
                                                                          output_normals, view_matrix, intrinsics)
            if args.weight_by_percent_pixels:
            ...
            if use_disc and args.patch_disc and args.patch_size < args.style_height:
                ...
                if weight_color is not None:
                    ...
                    weight_color_disc = args.weight_missing_color * weight_color_disc / torch.max(
                        weight_color_disc).item()
            if num_valid > 2000 and (not args.patch_disc or torch.sum(valid).item() > 0):
                if compute_2dstyle or compute_2dcontent:
                    ...
                    if args.color_space == 'lab':  # convert to rgb
                        ...
                        style_tgt_color = data_util.convert_lab01_to_rgb_pt(style_tgt_color).permute(0, 3, 1,
                                                                                                     2).contiguous()
                    else:
                        style_pred_color = style_pred_color.permute(0, 3, 1, 2).contiguous()
                        style_tgt_color = style_tgt_color.permute(0, 3, 1, 2).contiguous()
                    loss_style, loss_content = style.compute_style_loss(style_pred_color, style_tgt_color, model_style,
                                                                        compute_2dstyle, compute_2dcontent,
                                                                        missing_mask)
                ...
                if use_disc:  # train disc
                    if pred_color:
                        synth, target = style.preprocess_rendered_target_images(raycast, torch.cat(
                            [images_color, images_normals], 1))
                    ...
                    if args.weight_by_percent_pixels:
                        if args.disc_loss_type != 'hinge':
                            ...
                            if weight_sample_tgt2d is not None:
                                weight_sample_tgt2d = torch.from_numpy(
                                    np.repeat(weight_sample_tgt2d.cpu().numpy(), repeats)).cuda()
                            weight_sample_pred2d = torch.from_numpy(
                                np.repeat(weight_sample_pred2d.cpu().numpy(), repeats)).cuda()
                        if weight_sample_tgt2d is not None:
        ...
        if iter % 10000 == 0:
            ...
            if use_disc:
                torch.save({'epoch': epoch, 'state_dict': disc.state_dict(), 'optimizer': optimizer_disc.state_dict()},
                           os.path.join(args.save, 'disc-iter%s-epoch%s.pth' % (iter, epoch)))
        if iter % 100 == 0:
            gc.collect()
        if output_visual:
            ...
            if output_sdf is not None and len(output_sdf[0]) > 0:
                for b in range(args.batch_size):
                    ...
                    if len(mask) > 0:
                        vis_pred_sdf[b] = [output_sdf[0].detach()[mask].cpu().numpy(),
                ...
                if num_valid > 2000 and (not args.patch_disc or torch.sum(valid).item() > 0):
                    if compute_2dstyle or compute_2dcontent:
                        ...
                        if args.color_space == 'lab':  # convert to rgb
                            ...
                            style_pred_color = style_pred_color.permute(0, 3, 1, 2).contiguous()
                            style_tgt_color = style_tgt_color.permute(0, 3, 1, 2).contiguous()
                        loss_style, loss_content = style.compute_style_loss(style_pred_color, style_tgt_color,
                                                                            model_style, compute_2dstyle,
                                                                            compute_2dcontent, missing_mask)
                        loss = loss + args.weight_style_loss * loss_style + args.weight_content_loss * loss_content
                        if compute_2dstyle:
            ...
            if output_visual:
                ...
                if output_sdf is not None and len(output_sdf[0]) > 0:
                    for b in range(args.batch_size):
                        mask = output_sdf[0][:, -1] == b
                        if len(mask) > 0:
                            vis_pred_sdf[b] = [output_sdf[0][mask].detach().cpu().numpy(),
                                               output_sdf[1].detach()[mask].squeeze().cpu().numpy()]
                        if output_color is not None:
                            vis_pred_color[b] = output_color[mask].cpu().numpy()
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        if has_val:
            print_log(log_file_val, epoch, iter, train_losses, train_lossocc, train_iouocc, train_losssdf,
                      ...
                      val_losssdf, val_lossdepth, val_losscolor, val_losssemantic, val_lossdisc, val_lossdisc_real,
                      val_lossdisc_fake,
        ...
        if args.weight_disc_loss > 0:
            torch.save({'epoch': epoch + 1, 'state_dict': disc.state_dict(), 'optimizer': optimizer_disc.state_dict()},
                       os.path.join(args.save, 'disc-epoch-%s.pth' % epoch))
    log_file.close()
    if has_val:
        log_file_val.close()
