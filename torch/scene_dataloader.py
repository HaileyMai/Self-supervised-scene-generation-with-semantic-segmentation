...
def collate_voxels(batch):
    ...
    semantics = None
    if batch[0]['semantic'] is not None:
        semantics = torch.stack([x['semantic'] for x in batch])
    images_depth = None
    images_color = None
    images_pose = None
    ...
    return {'name': names, 'input': input, 'sdf': sdfs, 'world2grid': world2grids, 'known': known, 'colors': colors,
            'semantics': semantics, 'images_depth': images_depth, 'images_color': images_color,
            'images_pose': images_pose, 'images_intrinsic': images_intrinsic, 'mask': mask}


class SceneDataset(torch.utils.data.Dataset):

    def __init__(self, files, input_dim, truncation, load_known, augment_rgb_scaling, aug_scale_range, color_truncation,
                 color_space, frame_info_path='', frame_path='', image_dims=(320, 256), subsamp2d_factor=1,
                 randomize_frames=True, load_depth=False, load_color=True, load_semantic=False, num_overfit=0,
                 target_path='', max_input_height=0, scene_subsample_factor=1):
        self.is_chunks = target_path == ''  # have target path -> full scene data
        if not target_path:
            if load_semantic:
                self.files = [(f, f.replace('__inc__', '__sem__')) for f in files if
                              (os.path.isfile(f) and os.path.isfile(f.replace('__inc__', '__sem__')))]
            else:
                self.files = [(f, f.replace('__inc__', '__cmp__')) for f in files if
                              (os.path.isfile(f) and os.path.isfile(f.replace('__inc__', '__cmp__')))]
        else:
    ...
    def __getitem__(self, idx):
        ...
        if max_input_dim[0] >= sdf.shape[0] or max_input_dim[1] >= sdf.shape[1] or max_input_dim[2] >= sdf.shape[2]:
            mask = np.logical_and(input[0][:, 0] < sdf.shape[0],
                                  np.logical_and(input[0][:, 1] < sdf.shape[1], input[0][:, 2] < sdf.shape[2]))
        ...
        if self.frame_info_path and self.frame_path:
            image_depth, image_color, image_pose, image_intrinsic, image_frameids = data_util.load_frames(
                [name], [world2grid], self.frame_info_path, self.frame_path, randomize_frames=self.randomize_frames,
                depth_image_dims=self.image_dims, color_image_dims=self.image_dims, color_normalization=None,
                load_depth=self.load_depth, load_color=self.load_color)
            ...
            if image_color is not None:
                ...
                if self.subsamp2d_factor > 1:
                    ...
                    image_color = torch.nn.functional.interpolate(image_color, scale_factor=1.0 / self.subsamp2d_factor,
                                                                  mode='bilinear', align_corners=False)
                    image_color = torch.nn.functional.interpolate(image_color, size=sz, mode='bilinear',
                                                                  align_corners=False)

        input_dense = data_util.sparse_to_dense_np(input[0], input[1][:, np.newaxis], sdf.shape[2], sdf.shape[1],
                                                   sdf.shape[0], -float('inf'))
        if (self.is_chunks and (self.input_dim[0] != 96 and self.input_dim[0] != 128 and self.input_dim[
            0] != 160)) or self.scene_subsample_factor > 1:
            scale_factor = float(self.input_dim[0]) / 128 if self.is_chunks else 1.0 / float(
                self.scene_subsample_factor)
            input_dense = torch.nn.functional.interpolate(torch.from_numpy(input_dense).unsqueeze(0).unsqueeze(0),
                                                          scale_factor=scale_factor) * scale_factor
            input_dense = input_dense[0, 0].numpy()
            input_colors = torch.nn.functional.interpolate(
                torch.from_numpy(input_colors).permute(3, 0, 1, 2).contiguous().unsqueeze(0).float(),
                scale_factor=scale_factor)
            input_colors = input_colors[0].permute(1, 2, 3, 0).contiguous().numpy().astype(np.uint8)
            sdf = torch.nn.functional.interpolate(torch.from_numpy(sdf).unsqueeze(0).unsqueeze(0),
                                                  scale_factor=scale_factor) * scale_factor
            sdf = sdf[0, 0].numpy()
            colors = torch.nn.functional.interpolate(
                torch.from_numpy(colors).permute(3, 0, 1, 2).contiguous().unsqueeze(0).float(),
                scale_factor=scale_factor)
            colors = colors[0].permute(1, 2, 3, 0).contiguous().numpy().astype(np.uint8)
            if known is not None:
                known = torch.nn.functional.interpolate(torch.from_numpy(known).float().unsqueeze(0).unsqueeze(0),
