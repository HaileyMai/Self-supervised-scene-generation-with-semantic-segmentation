...
def load_sdf(file, load_sparse, load_known, load_color, is_sparse_file=True, color_file=None, load_semantic=False):
    ...
    known = None
    num_known = 0
    if load_color and color_file is None:  # chunk file
        ...
        if color_file is not None:
            with open(color_file, 'rb') as cfin:
                ...
                if is_sparse_file:
                    ...
                    color = struct.unpack('B' * num * 3, cfin.read(num * 3))
                    color = np.asarray(color, dtype=np.uint8).reshape(num, 3)
                    # TODO always loads dense
                    color = sparse_to_dense_np(locs, color, cdimx, cdimy, cdimz, 0)
                else:
                    color = struct.unpack('B' * cdimz * cdimy * cdimx * 3, cfin.read(cdimz * cdimy * cdimx * 3))
                    color = np.asarray(color, dtype=np.uint8).reshape([cdimz, cdimy, cdimx, 3])
        else:
...
def load_known(file, pad_known, pad_scan=(3, 6, 6), scale_to_dims=None):
    ...
    if scale_to_dims is not None and known.shape != scale_to_dims:
        ...
        assert (np.all(np.abs((known.shape[0] - scale_to_dims[0], known.shape[1] - scale_to_dims[1],
                               known.shape[2] - scale_to_dims[2])) < 2))
        known = known[:scale_to_dims[0], :scale_to_dims[1], :scale_to_dims[2]]
    return known
...
def visualize_colorgrid_as_points(colorgrid, output_file, thresh=15, transform=None):
    if np.max(colorgrid) < 1.1:
        print('[visualize_colorgrid_as_points] error: color range (', np.min(colorgrid), ',', np.max(colorgrid),
              ') expected [0,255]')
    # collect verts
    verts = []
    colors = []
...
def vis_semantic_as_hsv(semantics):
    ...
    semantics = np.stack([np.ones(semantics.shape[0]), semantics, np.ones(semantics.shape[0]) * 0.5], 1)
...
def convert_rgb_to_lab_pt(colorgrid):
    ...
    colors = torch.stack([116.0 * colorgrid[:, 1] - 16.0, 500.0 * (colorgrid[:, 0] - colorgrid[:, 1]),
                          200.0 * (colorgrid[:, 1] - colorgrid[:, 2])], 1)
...
def save_predictions(output_path, indices, names, inputs, target_for_sdf, target_for_colors, target_for_semantics,
        ...
        if target_color_images is not None:
            for k in range(len(target_color_images)):
                ...
                if output_color_images[k] is not None:
                    output_color_images[k] = convert_lab_to_rgb(output_color_images[k][np.newaxis, np.newaxis, :])[
                        0, 0].astype(np.uint8)
    ...
    for k in range(len(indices)):
        ...
        if target_color_images is not None and target_color_images[k] is None and target_for_colors is not None and \
                target_for_colors.shape[1] == 1:
            # vis input/pred/tgt images
            input = np.clip(
                255 * sparse_to_dense_np(inputs[0][inputs[0][:, -1] == k][:, :-1], feats[:, 1:], dims[2], dims[1],
...
def load_frame(depth_file, color_file, camera_file, depth_image_dims, color_image_dims, normalize, load_depth,
               load_color, intrinsic_file=None):
...
def load_frames(names, world2grids, frame_path, image_path, randomize_frames, depth_image_dims, color_image_dims,
                color_normalization, load_depth, load_color, max_num_frames=1):
    ...
    if len(frames[0]) < max_num_frames:
        ...
        return None, None, None, None, None
    depth_files = [[os.path.join(image_path, scene + '/depth/' + str(f) + '.png') for f in frameids] for frameids, scene
                   in zip(frames, scenes)]
    color_files = [[os.path.join(image_path, scene + '/color/' + str(f) + '.jpg') for f in frameids] for frameids, scene
                   in zip(frames, scenes)]
    camera_files = [[os.path.join(image_path, scene + '/camera/' + str(f) + '.txt') for f in frameids] for
                    frameids, scene in zip(frames, scenes)]
    depths = None if not load_depth else torch.zeros(batch_size, max_num_frames, depth_image_dims[1],
                                                     depth_image_dims[0], dtype=torch.float)
    colors = None if not load_color else torch.zeros(batch_size, max_num_frames, 3, color_image_dims[1],
                                                     color_image_dims[0], dtype=torch.float)
    for b in range(batch_size):
        for f in range(max_num_frames):
            depth_image, color_image, pose, intrinsic = load_frame(depth_files[b][f], color_files[b][f],
                                                                   camera_files[b][f], depth_image_dims,
                                                                   color_image_dims, color_normalization, load_depth,
                                                                   load_color)
            if load_depth:
